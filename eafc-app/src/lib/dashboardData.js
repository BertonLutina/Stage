...
export function getMatchOpponent(match, player, club) {
  ...
  const isHome = Boolean(isHomeClub || isHomePlayer);
...
export async function loadActiveContract(player, club) {
  ...
  const contracts = await stageClient.entities.PlayerContract.filter(
    { user_id: player.id },
    '-created_date',
    30,
  ).catch(() => []);
...
async function loadRankingsSummary() {
  try {
    ...
    if (Array.isArray(data)) return { players: data, clubs: [] };
    return {
      players: data?.players || data?.data?.players || [],
      clubs: data?.clubs || data?.data?.clubs || [],
    };
  } catch {
...
export async function loadPlayerDashboard() {
  const rankingsPromise = loadRankingsSummary();
  ...
  const [
    ...
    loadActiveTournaments(player, club),
