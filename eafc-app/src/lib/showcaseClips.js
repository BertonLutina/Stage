...
export function validateShowcaseDuration(durationSeconds) {
  if (!Number.isFinite(durationSeconds) || durationSeconds <= 0) {
    return { ok: false, error: 'Could not read that video. Try another clip.' };
  }
  if (durationSeconds > MAX_SHOWCASE_SECONDS) {
    return { ok: false, error: `Showcase clips must be ${MAX_SHOWCASE_SECONDS} seconds or shorter.` };
  }
...
export function validateShowcaseFileSize(bytes) {
  ...
  if (bytes > MAX_SHOWCASE_BYTES) {
    return { ok: false, error: `Showcase clips must be ${MAX_SHOWCASE_MB} MB or smaller.` };
  }
  return { ok: true };
}

export function showcaseVideoUri(video) {
  return video?.media_url || video?.url || null;
}
