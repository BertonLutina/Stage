...
function makeEmptyStats() {
  return { matches: 0, goals: 0, assists: 0, avgRating: null };
}
...
export function buildClubLeaderboard(players, stat, statsByPlayerId) {
  return (players || [])
    .filter((player) => player?.id)
    .map((player) => ({ player, value: getClubStatValue(player, stat, statsByPlayerId) }))
    .sort((a, b) => {
...
export const CLUB_STATS_TABLES = [
