...
//
// Expo / eafc-app: talks to Stage League native API at /api/stage
import '@/lib/polyfillStorage';
import { localStorage, sessionStorage } from '@/lib/polyfillStorage';
...
// Absolute URL required on React Native (no Vite proxy).
...
// Keep legacy eafc-app tokenService keys in sync so axios screens keep working.
const EAFC_ACCESS_KEY = 'eafc_access_token';
const EAFC_REFRESH_KEY = 'eafc_refresh_token';

function syncEafcTokenKeys() {
  try {
    const access = localStorage.getItem(ACCESS_KEY);
    const refresh = localStorage.getItem(REFRESH_KEY);
    if (access) localStorage.setItem(EAFC_ACCESS_KEY, access);
    else localStorage.removeItem(EAFC_ACCESS_KEY);
    if (refresh) localStorage.setItem(EAFC_REFRESH_KEY, refresh);
    else localStorage.removeItem(EAFC_REFRESH_KEY);
  } catch {
    /* ignore */
  }
}
...
function notifyAuthChanged() {
  // RN/Hermes has no DOM Event constructor — never use `new Event(...)`.
  try {
    if (typeof window === 'undefined') return;
    if (typeof window.dispatchEvent !== 'function') return;
    let event = null;
    if (typeof CustomEvent === 'function') {
      event = new CustomEvent(AUTH_CHANGED_EVENT);
    } else if (typeof Event === 'function') {
      event = new Event(AUTH_CHANGED_EVENT);
    } else {
      event = { type: AUTH_CHANGED_EVENT };
    }
    window.dispatchEvent(event);
  } catch {
    /* ignore — auth listeners are optional on native */
  }
}

// ── Token helpers ──────────────────────────────────────────────────────────────
export async function storeTokens({ accessToken, refreshToken, userId, playerId, ownerId, ownedClubId, presidentClubId, presidentId } = /** @type {any} */({})) {
  ...
  if (accessToken) {
    try {
      const m = await import('@/services/tokenService');
      await m.setTokens(accessToken, refreshToken || localStorage.getItem(REFRESH_KEY));
    } catch {
      /* ignore */
    }
  }
...
export async function clearTokens() {
  ...
  [ACCESS_KEY, REFRESH_KEY, USER_KEY, PLAYER_KEY, OWNER_KEY, PRESIDENT_CLUB_KEY, PRESIDENT_ID_KEY, EAFC_ACCESS_KEY, EAFC_REFRESH_KEY].forEach(k => localStorage.removeItem(k));
...
const auth = {
  ...
  async loginViaEmailPassword(identifier, password) {
    ...
    try {
      ...
      if (!res.ok) throw { status: res.status, message: data.error || data.message || 'Invalid credentials', ...data };
      await storeTokens(data);
      return { access_token: data.accessToken, ...data };
    } catch (err) {
  ...
  async registerViaEmailPassword({ email, password }) {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw { status: res.status, message: data.error || data.message || 'Register failed', ...data };
      await storeTokens(data);
  ...
  logout(redirectUrl) {
    ...
    clearTokens().catch(() => {});
    // Web only — Expo navigates via expo-router / auth store
    try {
  ...
  loginWithProvider(provider, returnTo) {
    ...
    const url = `${API_BASE}/auth/${provider}?client=mobile`;
    // Prefer Expo auth session when available; fall back to location assign on web.
    import('@/hooks/useSocialAuth')
      .then((m) => m.openSocialAuth(provider))
      .catch(() => {
  ...
  async handleOAuthCallback() {
    ...
    if (accessToken && (userId || playerId)) {
      await storeTokens({ accessToken, refreshToken, userId, playerId, ownerId, ownedClubId, presidentClubId, presidentId: params.get('presidentId') });
      ...
      try { window.history.replaceState({}, '', '/auth/callback'); } catch { /* RN */ }
