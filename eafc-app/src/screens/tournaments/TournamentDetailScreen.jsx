...
export default function TournamentDetailScreen({ route, navigation }) {
  ...
  const goToBracket = () => {
    ...
    if (format === 'group_knockout') navigation.navigate('GroupStage', { tournamentId });
    else if (format === 'classic_league' || format === 'league_playoffs') navigation.navigate('LeagueStandings', { tournamentId });
    else navigation.navigate('Bracket', { tournamentId });
  };
    ...
    <SafeAreaView className="flex-1 bg-dark">
      <ScrollView className="px-4">
        <View className="mt-4 mb-6">
          ...
          <Text className="text-muted text-sm mt-1">{tournament.description}</Text>
        </View>
        ...
        <View className="bg-card border border-border rounded-2xl p-4 mb-4">
          ...
          {(!tournament.teams || tournament.teams.length === 0) && <Text className="text-muted text-sm">No teams joined yet</Text>}
        </View>

        <View className="gap-3">
          {tournament.status !== 'draft' && (
            <Button title="View Bracket / Standings" variant="accent" onPress={goToBracket} />
          )}
          <Button title="Fixtures" variant="ghost" onPress={() => navigation.navigate('Fixtures', { tournamentId })} />
          {isOwner && tournament.status === 'draft' && (
            <Button title="Start Tournament" variant="secondary" onPress={async () => { await start(tournamentId); api.get(`/tournaments/${tournamentId}`).then(r => setTournament(r.data.data)); }} />
          )}
        </View>
      </ScrollView>
    </SafeAreaView>
