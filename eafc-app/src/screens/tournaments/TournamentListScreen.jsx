...
export default function TournamentListScreen({ navigation }) {
  ...
  useEffect(() => {
    api.get('/tournaments/list').catch(() => setTournaments([]));
  }, []);
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity onPress={() => navigation.navigate('TournamentDetail', { tournamentId: item.id })}
      ...
      <Text className="text-muted text-sm mt-1">{FORMAT_LABEL[item.format] || item.format}</Text>
  ...
  return (
    <SafeAreaView className="flex-1 bg-dark">
      <View className="flex-row justify-between items-center px-4 py-4">
        ...
        <Button title="+ New" onPress={() => navigation.navigate('CreateTournament')} className="py-2 px-4" />
      </View>
